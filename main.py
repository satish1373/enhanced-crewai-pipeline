"""
CrewAI Automation Pipeline - Corrected Version
...
router = Agent(
    ...
    goal="Analyze Jira tickets and determine the best development approach and complexity level",
    backstory="An experienced technical lead who can quickly assess requirements and route tasks appropriately.",
    verbose=True
)
...
def extract_code_blocks(text) -> list:
...
def fetch_new_ticket(jql: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        jql = jql or f'project = {JIRA_PROJECT_KEY} AND status = "To Do" ORDER BY created DESC'
...
def record_attempt(workflow_summary: dict, ticket_key: str, summary: str, 
    ...
    # Extract router analysis from log if it's the first attempt
    if not workflow_summary[ticket_key]["router_analysis"] and "Router Analysis:" in log_snippet:
        router_section = log_snippet.split("Router Analysis:")[1].split("Coder Output:")[0].strip()
        workflow_summary[ticket_key]["router_analysis"] = router_section
...
def process_ticket(ticket: Any, processed_tickets: set, workflow_summary: dict):
    ...
    # Step 0: Router analyzes the ticket and determines approach
    try:
        router_task = Task(
            description=f"""Analyze this Jira ticket and provide development guidance:
...
2. Estimated development approach
3. Key requirements and considerations
4. Recommended test cases
5. Any potential challenges or edge cases

Respond in a structured format with clear sections.""",
            agent=router,
            expected_output="A structured analysis with complexity level, development approach, requirements, test cases, and potential challenges"
        )
        router_crew = Crew(agents=[router], tasks=[router_task])
        router_result = router_crew.kickoff()
        
        # Convert CrewOutput to string
        if hasattr(router_result, 'raw'):
            router_result = router_result.raw
        elif not isinstance(router_result, str):
            router_result = str(router_result)
        
        logger.info("Router analysis completed")
        
        # Enhanced task description with router insights
        enhanced_description = f"""Original Request: {task_description}

Router Analysis:
{router_result}

Based on the above analysis, implement the requested functionality."""
        
    except Exception as e:
        logger.error(f"Router agent failed: {e}")
        enhanced_description = task_description
        router_result = f"Router analysis failed: {e}"
    ...
    try:
        coding_task = Task(
            ...
            agent=coder,
            expected_output="Complete Python code that implements the requested functionality with proper structure and comments"
        )
        crew = Crew(agents=[coder], tasks=[coding_task])
        coder_result = crew.kickoff()
        
        # Convert CrewOutput to string
        if hasattr(coder_result, 'raw'):
            coder_result = coder_result.raw
        elif not isinstance(coder_result, str):
            coder_result = str(coder_result)
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        try:
            review_task_template = """Review the following Python code considering the router analysis:
...
2. Wrap in main() with comprehensive test cases based on router recommendations
3. Include test cases for edge cases identified by the router
4. Print test results with checkmark or X indicators
5. Correct failing tests automatically.
6. Return final refactored code."""
            
            review_task = Task(
                description=review_task_template.format(
                    router_analysis=router_result,
                    code=refactored_code
                ),
                agent=reviewer,
                expected_output="Refactored Python code wrapped in main() function with comprehensive test cases and proper error handling"
            )
            review_crew = Crew(agents=[reviewer], tasks=[review_task])
            review_result = review_crew.kickoff()
            
            # Convert CrewOutput to string
            if hasattr(review_result, 'raw'):
                review_result = review_result.raw
            elif not isinstance(review_result, str):
            ...
            qa_result = ""
            try:
                qa_task_template = """Check the following Python code for quality, considering the router analysis:
...
3. Validate all tests pass and cover edge cases from router analysis
4. Verify implementation meets requirements from router analysis
5. Respond with either "APPROVED" or "REJECTED" followed by your comments"""
                
                qa_task = Task(
                    description=qa_task_template.format(
                        router_analysis=router_result,
                        code=refactored_code
                    ),
                    agent=qa_agent,
                    expected_output="Quality assessment with APPROVED/REJECTED decision and detailed comments on code quality, security, and compliance"
                )
                ...
                if hasattr(qa_result, 'raw'):
                    qa_result = qa_result.raw
                elif not isinstance(qa_result, str):
            ...
            # Save logs including router analysis
            with open("crewai_output.txt", "w", encoding="utf-8") as f:
                full_log = f"""Router Analysis:
{router_result}
